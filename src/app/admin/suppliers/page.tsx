...
export default function AdminSuppliersPage() {
  ...
  async function loadSuppliers() {
    ...
    try {
      ...
      const res = await fetch("/api/teklifim-gelsin/admin/suppliers", {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            Tedarikçi Operasyon Masası
          </h1>
          <p className="text-sm text-slate-400 mt-1">
            Platformdaki tüm onaylı ve aday toptancıların listesi, performans metrikleri ve yönetim aksiyonları.
          </p>
        </div>
        <button
      ...
      <div className="glass-panel p-4 rounded-xl border border-slate-800 bg-slate-900/60">
        <div className="flex items-center gap-2 max-w-md bg-slate-950/60 border border-slate-800 rounded-lg px-3 py-2">
          ...
          <input
            ...
            placeholder="Tedarikçi adı, şehir veya e-posta ara..."
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 text-slate-400 uppercase tracking-wider font-semibold border-b border-slate-800">
              <tr>
                ...
                <th className="px-4 py-3">Puan & İşlemler</th>
                <th className="px-4 py-3">Doğrulama</th>
                <th className="px-4 py-3">Durum</th>
                <th className="px-4 py-3 text-right">Detay & Aksiyon</th>
              </tr>
            </thead>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-slate-500">
                    {loading ? "Tedarikçiler getiriliyor..." : "Tedarikçi kaydı bulunamadı."}
                  </td>
                </tr>
              ) : (
                filtered.map((s) => (
                  <tr key={s.uid} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-semibold text-white">{s.companyName || "İsimsiz Firma"}</div>
                    ...
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[11px] text-slate-400">
                        {s.completedDeals || 0} tamamlanan sipariş
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      {s.isVerified ? (
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        href={`/admin/users?search=${encodeURIComponent(s.email)}`}
