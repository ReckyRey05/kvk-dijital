    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3">
        {deliveryPlatforms.map((p) => {
          ...
          return (
            <div
              key={p.platform}
              className={`p-3 sm:p-4 rounded-2xl bg-[#0a0f0f] border transition-all ${
                p.isOpen ? "border-white/10 hover:border-white/20" : "border-white/5 opacity-60"
              }`}
            >
              <div className="flex items-center justify-between gap-1">
                <span className={`px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-xl text-[10px] sm:text-[11px] font-extrabold border truncate ${badge.color}`}>
                  {badge.name}
                </span>
              ...
              <div className="mt-2.5 sm:mt-3 pt-2 border-t border-white/5 flex items-center justify-between text-[10px] sm:text-[11px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        <div className="flex items-center gap-2 overflow-x-auto sleek-scrollbar pb-1 sm:pb-0">
          ...
          >
            Tüm Geçmiş ({orders.length})
          </button>
        </div>

        <div className="text-[11px] sm:text-xs font-bold text-foreground/60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 sm:gap-4">
        {activeOrders.map((order) => {
          ...
          return (
            <div
              key={order.id}
              className={`p-3.5 sm:p-5 rounded-2xl bg-[#0a0f0f] border transition-all ${
                isPending
            ...
            >
              {/* Order Header */}
              <div className="flex flex-wrap xs:flex-nowrap items-center justify-between pb-3 border-b border-white/10 gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={`px-2.5 py-1 rounded-xl text-[10px] font-extrabold border shrink-0 ${badge.color}`}>
                    {badge.name}
                  </span>
                  <span className="font-mono text-xs font-bold text-white truncate">{order.platformOrderId}</span>
                </div>
              ...
              <div className="py-3 border-b border-white/5 space-y-1.5 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 text-foreground/70 font-mono text-[11px] shrink-0">
                ...
                <div className="flex items-start gap-1.5 text-foreground/80 text-[11px]">
                  ...
                  <span className="leading-snug break-words">{order.deliveryAddress}</span>
                </div>

                {order.courierNotes && (
                  <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-[10px] text-amber-300 font-semibold break-words">
              ...
              <div className="py-3 space-y-1.5">
                {order.items.map((it, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="w-5 h-5 rounded-md bg-white/10 text-white font-bold text-[10px] flex items-center justify-center shrink-0">
                        {it.quantity}x
                      </span>
                      <span className="text-white font-medium truncate">{it.name}</span>
                    </div>
                    <span className="font-bold text-foreground/80 shrink-0 ml-2">{it.finalPrice * it.quantity} TL</span>
                  </div>
                ))}
              </div>
              ...
              <div className="pt-3 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div>
                  <span className="text-[10px] text-foreground/50 block">Ödeme: {order.paymentType === "ONLINE_PAID" ? "Online Ödendi" : "Kapıda Tahsilat"}</span>
                  <span className="text-sm sm:text-base font-black text-emerald-400">{order.totalAmount.toLocaleString("tr-TR")} TL</span>
                </div>

                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <button
                    onClick={() => handlePrintCourierReceipt(order)}
                    className="p-2 sm:p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white transition-colors cursor-pointer shrink-0"
                  ...
                  {isPending && (
                    <button
                      ...
                      className="flex-1 sm:flex-none px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-accent text-black font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-accent/20 hover:bg-accent/90 transition-all cursor-pointer"
                    >
                  ...
                  {isPreparing && (
                    <button
                      ...
                      className="flex-1 sm:flex-none px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-500/20 transition-all cursor-pointer"
                    >
