  ...
  return (
    <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-4">
      {tables.map((table) => {
        ...
        const activeCalls = waiterCalls.filter(
        ...
        // Status styling
        const isBillReq = table.status === "BILL_REQUESTED" || activeCalls.some((c) => c.type.startsWith("BILL"));
        const isWaiterCalled = table.status === "WAITER_CALLED" || activeCalls.length > 0;
        ...
        return (
          ...
          >
            ...
            {/* Active Waiter Call Notification Banners (Preserves Concurrent Requests) */}
            {activeCalls.length > 0 && (
              <div className="space-y-1 my-1 max-h-20 overflow-y-auto sleek-scrollbar">
                {activeCalls.map((call) => (
                  <div
                    ...
                    className="p-1.5 rounded-xl bg-purple-500/20 border border-purple-500/30 flex items-center justify-between text-purple-200 text-[10px] hover:bg-purple-500/30 transition-colors"
                    title="Talebi Tamamla"
                  >
                    <div className="flex items-center gap-1.5 truncate">
                      <Bell className="w-3 h-3 text-purple-400 shrink-0 animate-bounce" />
                      <span className="truncate text-[10px] font-semibold">{call.message || call.type}</span>
                    </div>
